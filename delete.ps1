##################################################
# HelloID-Conn-Prov-Target-WinkWaves-Delete
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting WinkWaves account with accountReference: [$($actionContext.References.Account)]"
                ...
                Write-Information "[DryRun] Delete WinkWaves account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete WinkWaves account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete WinkWaves account. Error: $($_.Exception.Message)"
